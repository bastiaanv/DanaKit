...
public enum DanaKitPumpManagerError: Error {
    case noPodPaired
...
extension DanaKitPumpManagerError: LocalizedError {
    public var errorDescription: String? {
        ...
        case .insulinTypeNotConfigured:
            return LocalizedString("Insulin type not configured", comment: "Error description for DanaKitPumpManagerError.insulinTypeNotConfigured")
        case .notReadyForCannulaInsertion:
        ...
        case .invalidSetting:
            return LocalizedString("Invalid Setting", comment: "Error description for DanaKitPumpManagerError.invalidSetting")
        }
    }
...
public class DanaKitPumpManager: DeviceManager {
    ...
    public let localizedTitle = LocalizedString("Omnipod DASH", comment: "Generic title of the DanaKit pump manager")

    static let podAlarmNotificationIdentifier = "DanaKit:\(LoopNotificationCategory.pumpFault.rawValue)"
    ...
    private func setState(_ changes: (_ state: inout DanaKitPumpManagerState) -> Void) -> Void {
        return setStateWithResult(changes)
    }

    @discardableResult
    private func mutateState(_ changes: (_ state: inout DanaKitPumpManagerState) -> Void) -> DanaKitPumpManagerState {
        return setStateWithResult({ (state) -> DanaKitPumpManagerState in
            changes(&state)
            return state
        })
    }
    ...
    private func setStateWithResult<ReturnType>(_ changes: (_ state: inout DanaKitPumpManagerState) -> ReturnType) -> ReturnType {
        var oldValue: DanaKitPumpManagerState!
    ...
    public let log = OSLog(category: "DanaKitPumpManager")
...
extension DanaKitPumpManager {
    ...
    private func status(for state: DanaKitPumpManagerState) -> PumpManagerStatus {
    ...
    private func device(for state: DanaKitPumpManagerState) -> HKDevice {
        if let podState = state.podState {
            return HKDevice(
                ...
                softwareVersion: String(DanaKitVersionNumber),
    ...
    private func basalDeliveryState(for state: DanaKitPumpManagerState) -> PumpManagerStatus.BasalDeliveryState {
    ...
    private func bolusState(for state: DanaKitPumpManagerState) -> PumpManagerStatus.BolusState {
        guard let podState = state.podState else {
            return .noBolus
        }
    ...
    private func podCommState(for state: DanaKitPumpManagerState) -> PodCommState {
        guard let podState = state.podState else {
            return .noPod
        }
    ...
    public func buildPumpStatusHighlight(for state: DanaKitPumpManagerState, andDate date: Date = Date()) -> PumpStatusHighlight? {
    ...
    public func isRunningManualTempBasal(for state: DanaKitPumpManagerState) -> Bool {
    ...
    public func buildPumpLifecycleProgress(for state: DanaKitPumpManagerState) -> PumpLifecycleProgress? {
        switch podCommState {
        case .active:
            if shouldWarnPodEOL,
    ...
    func connectToNewPod(completion: @escaping (Result<DanaKit, Error>) -> Void) {
    ...
    public func pairAndPrime(completion: @escaping (PumpManagerResult<TimeInterval>) -> Void) {
        ...
        if needsPairing {
            ...
            guard let insulinType = insulinType else {
                completion(.failure(.configuration(DanaKitPumpManagerError.insulinTypeNotConfigured)))
                return
            }
    ...
    // Called on the main thread
    public func insertCannula(completion: @escaping (Result<TimeInterval,DanaKitPumpManagerError>) -> Void) {
        
        #if targetEnvironment(simulator)
        let mockDelay = TimeInterval(seconds: 3)
        let mockFaultDuringInsertCannula = false
        DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + mockDelay) {
            let result = self.setStateWithResult({ (state) -> Result<TimeInterval,DanaKitPumpManagerError> in
                if mockFaultDuringInsertCannula {
                    ...
                    podState?.fault = fault
                    state.updatePodStateFromPodComms(podState)
                    return .failure(DanaKitPumpManagerError.communication(PodCommsError.podFault(fault: fault)))
                }
        ...
        #else
        let preError = setStateWithResult({ (state) -> DanaKitPumpManagerError? in
    ...
    public func checkCannulaInsertionFinished(completion: @escaping (DanaKitPumpManagerError?) -> Void) {
        #if targetEnvironment(simulator)
        completion(nil)
        #else
    ...
    public func getPodStatus(completion: ((_ result: PumpManagerResult<StatusResponse>) -> Void)? = nil) {
        guard state.hasActivePod else {
            completion?(.failure(PumpManagerError.configuration(DanaKitPumpManagerError.noPodPaired)))
            return
        }
    ...
    public func setTime(completion: @escaping (DanaKitPumpManagerError?) -> Void) {

        guard state.hasActivePod else {
            completion(DanaKitPumpManagerError.noPodPaired)
            return
        }
    ...
    public func deactivatePod(completion: @escaping (DanaKitPumpManagerError?) -> Void) {
        #if targetEnvironment(simulator)
        DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + .seconds(2)) {
            completion(nil)
        }
        ...
        self.podComms.runSession(withName: "Deactivate pod") { (result) in
            ...
            case .success(let session):
                do {
                    ...
                    completion(DanaKitPumpManagerError.communication(error))
                }
            case .failure(let error):
                completion(DanaKitPumpManagerError.communication(error))
            }
    ...
    public func readPulseLog(completion: @escaping (Result<String, Error>) -> Void) {
        ...
        guard self.hasSetupPod else {
            completion(.failure(DanaKitPumpManagerError.noPodPaired))
            return
        }
    ...
    public func setConfirmationBeeps(newPreference: BeepPreference, completion: @escaping (DanaKitPumpManagerError?) -> Void) {
...
extension DanaKitPumpManager: PumpManager {
    ...
    public func enactBolus(units: Double, activationType: BolusActivationType, completion: @escaping (PumpManagerError?) -> Void) {
        guard self.hasActivePod else {
            completion(.configuration(DanaKitPumpManagerError.noPodPaired))
            return
        }
    ...
    public func cancelBolus(completion: @escaping (PumpManagerResult<DoseEntry?>) -> Void) {
        guard self.hasActivePod else {
            completion(.failure(.deviceState(DanaKitPumpManagerError.noPodPaired)))
            return
        }
    ...
    public func runTemporaryBasalProgram(unitsPerHour: Double, for duration: TimeInterval, automatic: Bool, completion: @escaping (PumpManagerError?) -> Void) {
        guard self.hasActivePod else {
            completion(.deviceState(DanaKitPumpManagerError.noPodPaired))
            return
        }
    ...
    public func syncDeliveryLimits(limits deliveryLimits: DeliveryLimits, completion: @escaping (Result<DeliveryLimits, Error>) -> Void) {
        mutateState { state in
            if let rate = deliveryLimits.maximumBasalRate?.doubleValue(for: .internationalUnitsPerHour) {
                ...
                completion(.failure(DanaKitPumpManagerError.invalidSetting))
            }
        }
    }
    ...
    public func updateExpirationReminder(_ intervalBeforeExpiration: TimeInterval?, completion: @escaping (DanaKitPumpManagerError?) -> Void) {
    ...
    public func updateLowReservoirReminder(_ value: Int, completion: @escaping (DanaKitPumpManagerError?) -> Void) {
        guard self.hasActivePod else {
            completion(DanaKitPumpManagerError.noPodPaired)
            return
        }
    ...
    private func notifyPodFault(fault: DetailedStatus) {
        pumpDelegate.notify { delegate in
            ...
            delegate?.issueAlert(Alert(identifier: Alert.Identifier(managerIdentifier: DanaKitPumpManager.podAlarmNotificationIdentifier,
...
extension DanaKitPumpManager: MessageLogger {
...
extension DanaKitPumpManager: PodCommsDelegate {

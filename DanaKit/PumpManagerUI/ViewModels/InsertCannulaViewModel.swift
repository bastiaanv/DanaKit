...
public protocol CannulaInserter {
    func insertCannula(completion: @escaping (Result<TimeInterval,DanaKitPumpManagerError>) -> ())
    func checkCannulaInsertionFinished(completion: @escaping (DanaKitPumpManagerError?) -> Void)
}

extension DanaKitPumpManager: CannulaInserter { }
...
class InsertCannulaViewModel: ObservableObject, Identifiable {
    ...
    enum InsertCannulaViewModelState {
        ...
        case error(DanaKitPumpManagerError)
        case finished
    ...
    var error: DanaKitPumpManagerError? {
...
public extension DanaKitPumpManagerError {

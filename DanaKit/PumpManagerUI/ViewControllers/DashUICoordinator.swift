...
class DashUICoordinator: UINavigationController, PumpManagerOnboarding, CompletionNotifying, UINavigationControllerDelegate {
    ...
    private var pumpManagerType: DanaKitPumpManager.Type?
    ...
    private func viewControllerForScreen(_ screen: DashUIScreen) -> UIViewController {
        ...
        case .deactivate:
            ...
            return hostedView
        case .settings:
            let viewModel = DanaKitSettingsViewModel(pumpManager: pumpManager)
            viewModel.didFinish = { [weak self] in
                self?.stepFinished()
            }
            ...
            let view = DanaKitSettingsView(viewModel: viewModel, supportedInsulinTypes: allowedInsulinTypes)
            return hostingController(rootView: view)
        case .pairPod:
    ...
    init(pumpManager: DanaKitPumpManager? = nil, colorPalette: LoopUIColorPalette, pumpManagerSettings: PumpManagerSetupSettings? = nil, allowDebugFeatures: Bool, allowedInsulinTypes: [InsulinType] = [])
    {
        if pumpManager == nil, let pumpManagerSettings = pumpManagerSettings {
            ...
            let pumpManagerState = DanaKitPumpManagerState(podState: nil, timeZone: basalSchedule.timeZone, basalSchedule: BasalSchedule(repeatingScheduleValues: basalSchedule.items), insulinType: nil, maximumTempBasalRate: pumpManagerSettings.maxBasalRateUnitsPerHour)
            self.pumpManager = DanaKitPumpManager(state: pumpManagerState)
        } else {
